...
const CustomerServiceManagement = () => {
  ...
  return (
    <Fragment>
      <Breadcrumbs title='고객센터 관리' data={[{ title: '운영' }, { title: '고객센터 관리' }]} />
      <Row>
        <Col sm='12'>
          <Nav tabs>
            <NavItem>
          ...
          <TabContent activeTab={activeTab}>
            ...
            <TabPane tabId='service_guide'>
              {activeTab === 'service_guide' && <ContentTab type="service_guide" title="서비스 가이드" />}
            </TabPane>
            <TabPane tabId='terms_of_use'>
              {activeTab === 'terms_of_use' && <ContentTab type="terms_of_use" title="이용약관" />}
            </TabPane>
            <TabPane tabId='privacy_policy'>
              {activeTab === 'privacy_policy' && <ContentTab type="privacy_policy" title="개인정보처리방침" />}
            </TabPane>
          </TabContent>
        </Col>
      </Row>
