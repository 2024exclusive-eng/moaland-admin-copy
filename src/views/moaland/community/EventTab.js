...
const EventTab = () => {
  ...
  const handleSaveReorder = async () => {
    try {
      const events = reorderedList.map((event, index) => ({
        id: event.id,
        order: index + 1,
      }));

      await axios.post("/admin/event/reorder", { events });
      setIsReorderMode(false);
      setReorderedList([]);
      loadData();
    } catch (error) {
      console.error("Error reordering events:", error);
      alert("이벤트 순서 변경에 실패했습니다.");
    }
  };
      ...
      <tr key={event.id}>
        {isReorderMode && (
          <td
            ...
            style={{ width: "57px", padding: "16px", cursor: "grab" }}
          >
            <DragHandleIcon />
          </td>
        )}
  ...
  const renderData = () => {
    ...
    return listToRender.map((event, index) => renderRow(event, index));
  };
    ...
    <Fragment>
      <Card className="event-tab-card mt-2">
        <div style={{ padding: "32px" }}>
          ...
          >
            <h4 className="mb-0">등록된 이벤트 {data.data.length}/6</h4>
            ...
            <Table className="event-table" responsive>
              ...
              {isReorderMode ? (
                <ReactSortable
                  ...
                  animation={200}
                >
                  {reorderedList.map((event, index) => renderRow(event, index))}
                </ReactSortable>
              ) : (
